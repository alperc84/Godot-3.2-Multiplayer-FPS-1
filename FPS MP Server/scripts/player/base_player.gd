...
func _physics_process(delta):
	...
	if state.dead and cmd.primary_fire and can_respawn:
		respawn()
...
func kick_raycast():
	...
	if result:
		...
		if result.collider is PhysicalBone:
			result.collider.apply_impulse(pos, dir * 5)
		...
		if result.collider is KinematicBody and result.collider.has_method("hit"):
			...
			result.collider.set_velocity(dir * 5)

func update_puppets():
	if !state.dead and networking.can_send:
