...
#		hvel = hvel.linear_interpolate(target, accel * delta)
		...
		if dir.dot(hvel) > 0:
			if state.grounded:
				...
				accel = vars.AIR_ACCEL
		else:
			if state.grounded:
				accel = vars.DEACCEL
			else:
				accel = vars.AIR_DECEL
		# Quake-like movement
		var current_speed = hvel.dot(dir)
		var add_speed : float
		if cmd.sprint:
			add_speed = vars.MAX_SPRINT_SPEED - current_speed
		else:
			add_speed = vars.MAX_SPEED - current_speed
		add_speed = clamp(add_speed, 0, accel * 10000 * delta)
		hvel += dir * add_speed
		# Deceleration
		if hvel.length() != 0:
			hvel -= hvel * accel * delta
		
		vel.x = hvel.x
		vel.z = hvel.z
		vel = move_and_slide(vel, Vector3.UP, 0.05, 4, deg2rad(vars.MAX_SLOPE_ANGLE), false)
...
func process_animations(delta):
	...
	if active_weapon_index == 0:
		anim_tree["parameters/blend_shotgun/blend_amount"] = 1.0
		anim_tree["parameters/blend_smg/blend_amount"] = 0.0
	elif active_weapon_index == 1:
		anim_tree["parameters/blend_shotgun/blend_amount"] = 0.0
		anim_tree["parameters/blend_smg/blend_amount"] = 1.0
	elif active_weapon_index == 2:
		anim_tree["parameters/blend_shotgun/blend_amount"] = 1.0
		anim_tree["parameters/blend_smg/blend_amount"] = 0.0
	
	if state.firing and active_weapon_index == 0:
		anim_tree["parameters/shot_shotgun_fire/active"] = true
	elif state.firing and active_weapon_index == 1:
		anim_tree["parameters/shot_smg_fire/active"] = true
	elif state.firing and active_weapon_index == 2:
		anim_tree["parameters/shot_shotgun_fire/active"] = true
