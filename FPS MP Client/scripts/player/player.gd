...
func _on_state_changed(s, _b):
	match s:
		"dead":
			...
			if state[s]:
				game.main.display_message("You are dead! Respawning...")
...
func process_input(_delta):
	...
	if Input.is_action_pressed("sprint"):
		cmd.sprint = true
		rpc_unreliable_id(1, "execute_command", "sprint", true)
	else:
		cmd.sprint = false
		rpc_unreliable_id(1, "execute_command", "sprint", false)
...
puppet func set_water_filter(value):
	water_filter = value
